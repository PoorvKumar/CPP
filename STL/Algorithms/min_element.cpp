...
int main()
{
    ...
    cout<<"Minimum element in array:"<<*min_element(arr,arr+9)<<" "<<min_element(arr,arr+9)<<endl;
    //min_element(beginAddress,endAddress) returns iterator object(for other containers like vector) where minimum value in container
    ...
    //min_element() useful for finding minimum element in a subarray or subsequence of a container

    cout<<"Vector:\n";
    vector<int> vec={3,5,7,9,2,1,4,0,14,16};
    print(vec,vec.size());

    cout<<"Minimum element in vector:"<<*min_element(vec.begin(),vec.end())<<endl; 
